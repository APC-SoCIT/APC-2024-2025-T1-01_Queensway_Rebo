...
<div class="auth-wrapper">
    <div class="auth-card" data-aos="fade-up">
        ...
        <div class="auth-right">
            <h2>{{ $isAdmin ? 'Reset Admin Password' : 'Reset User Password' }}</h2>
            ...
            <form method="POST" 
                action="{{ $isAdmin ? route('admin.password.update') : route('password.update') }}" 
                onsubmit="showResetSpinner(event)">
                @csrf
                <input type="hidden" name="token" value="{{ $token }}">

                <div class="mb-3">
                    <label for="email" class="form-label">Email Address</label>
                    <input type="email" id="email" name="email" value="{{ $email ?? old('email') }}" class="form-control" required>
                </div>
            ...
            <div class="link-container">
                <a href="{{ $isAdmin ? route('admin.login') : route('login') }}">← Back to Login</a>
            </div>
        </div>
    </div>
</div>
